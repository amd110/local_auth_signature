...
public class BiometricSignatureManager : SignatureManager {
    ...
    private let keyManager: KeyManager = KeyPairManager()
    
    public func sign(key: String, algorithm: SecKeyAlgorithm, data: Data) -> SignatureResult {
        let key = keyManager.get(name: key)
        guard key != nil else {
    ...
    public func sign(key: String, message: String) -> SignatureResult {
        return sign(
            key: key,
    ...
    public func verify(key: String, message: String, signature: String) -> Bool {
        let key = keyManager.get(name: key)
        guard key != nil else {
            print("Can't get or create key pair")
            return false
